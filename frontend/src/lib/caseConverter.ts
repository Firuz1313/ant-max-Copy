...
export function convertObjectKeys<T = any>(
  ...
  if (Array.isArray(obj)) {
    return obj.map(item => convertObjectKeys(item, converter)) as unknown as T;
  }
...
export function transformToFrontend<T = any>(data: any): T {
  ...
  return convertObjectKeys(data, (key: string) => {
    return FIELD_MAPPINGS.backend_to_frontend[key as keyof typeof FIELD_MAPPINGS.backend_to_frontend] || snakeToCamel(key);
  });
}
...
export function transformToBackend<T = any>(data: any): T {
  ...
  return convertObjectKeys(data, (key: string) => {
    return FIELD_MAPPINGS.frontend_to_backend[key as keyof typeof FIELD_MAPPINGS.frontend_to_backend] || camelToSnake(key);
  });
}
