  ...
  const runTests = async () => {
    ...
    try {
      ...
      testResults.push({
        ...
        message: response.connected ? 'Database connected successfully' : 'Database connection failed',
        data: response,
        duration
      });
    } catch (error: any) {
    ...
    try {
      ...
      testResults.push({
        name: 'Get Devices',
        success: isArray,
        message: isArray ? `Retrieved ${devices.length} devices` : 'Response is not an array',
        data: { count: isArray ? devices.length : 0, sample: isArray ? devices[0] : null },
        duration
      });
    } catch (error: any) {
      testResults.push({
    ...
    try {
      ...
      testResults.push({
        name: 'Get Problems',
        success: isArray,
        message: isArray ? `Retrieved ${problems.length} problems` : 'Response is not an array',
        data: { count: isArray ? problems.length : 0, sample: isArray ? problems[0] : null },
        duration
      });
    } catch (error: any) {
      testResults.push({
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      {results.length > 0 && (
        <Alert variant={allPassed ? "default" : "destructive"}>
          ...
          <AlertDescription>
            <strong>Test Results:</strong> {successCount} of {totalTests} tests passed
