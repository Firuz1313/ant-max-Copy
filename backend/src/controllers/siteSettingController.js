...
export const validateSettings = async (req, res) => {
  try {
    ...
    if (settingsData.maxStepsPerProblem && (settingsData.maxStepsPerProblem < 1 || settingsData.maxStepsPerProblem > 100)) {
      errors.push('Максимальное количество шагов должно быть от 1 до 100');
    }
    
    if (settingsData.maxMediaSize && (settingsData.maxMediaSize < 1 || settingsData.maxMediaSize > 100)) {
      errors.push('Максимальный размер медиа должен быть от 1 до 100 MB');
    }
    
    if (settingsData.sessionTimeout && (settingsData.sessionTimeout < 5 || settingsData.sessionTimeout > 1440)) {
      errors.push('Таймаут сессии должен быть от 5 до 1440 минут');
    }
    
    // Валидация цветов
    const colorRegex = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/;
    if (settingsData.primaryColor && !colorRegex.test(settingsData.primaryColor)) {
      errors.push('Основной цвет должен быть в формате HEX');
    }
    
    if (settingsData.accentColor && !colorRegex.test(settingsData.accentColor)) {
