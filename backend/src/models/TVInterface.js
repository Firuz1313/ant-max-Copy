...
class TVInterface extends BaseModel {
  ...
  getSchema() {
    return {
      ...
      type: { 
        type: 'string', 
        required: true,
        enum: ['home', 'settings', 'channels', 'apps', 'guide', 'no-signal', 'error', 'custom']
      },
  ...
  async create(data) {
    try {
      ...
      const deviceExists = await this.query('SELECT id FROM devices WHERE id = $1', [data.device_id]);
  ...
  async update(id, data) {
    try {
      ...
      if (data.screenshot_data !== undefined) updateData.screenshot_data = data.screenshot_data;
      if (data.clickable_areas !== undefined) updateData.clickable_areas = JSON.stringify(data.clickable_areas);
