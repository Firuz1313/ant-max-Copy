...
export class ApiValidator {
  ...
  static async validateDatabaseConnection() {
    try {
      ...
      return {
        ...
        message: result.success ? 'PostgreSQL connection successful' : 'PostgreSQL connection failed',
        details: result
      };
    } catch (error) {
      return {
  ...
  static async validateTableStructure() {
    try {
      ...
      const requiredTables = [
        'devices', 'problems', 'diagnostic_steps', 'remotes', 
        'tv_interfaces', 'tv_interface_marks', 'diagnostic_sessions',
        'session_steps', 'step_actions', 'change_logs', 'site_settings', 'users'
      ];
      ...
      return {
        ...
        message: missingTables.length === 0 ? 'All tables exist' : `Missing tables: ${missingTables.join(', ')}`,
        existingTables,
        missingTables,
  ...
  static async validateDataIntegrity() {
    try {
      ...
      for (const table of tables) {
        try {
          ...
          checks[table] = {
            ...
            sampleStructure: sampleResult.rows[0] ? Object.keys(sampleResult.rows[0]) : []
          };
        } catch (error) {
          checks[table] = {
            error: error.message,
      ...
      const totalRecords = Object.values(checks).reduce((sum, check) => sum + (check.count || 0), 0);
    ...
    try {
      ...
      for (const model of models) {
        try {
          ...
          const selectResult = await db.query(`SELECT COUNT(*) FROM ${model.table}`);
  ...
  static async runFullValidation() {
    ...
    // Overall success
    results.success = Object.values(results.validations).every(v => v.success);
    results.message = results.success ? 
      'All validations passed - API is ready' : 
      'Some validations failed - check details';

    console.log(results.success ? '✅ Validation completed successfully' : '❌ Validation completed with errors');
