...
export const up = async () => {
  ...
  try {
    ...
    console.log('1. Добавляем частичный UNIQUE индекс на devices.name (только для активных)...');
    await query(`
    ...
    console.log('2. Добавляем композитный UNIQUE индекс на problems (title, device_id)...');
    await query(`
    ...
    console.log('3. Добавляем композитный UNIQUE индекс на remotes (name, device_id)...');
    await query(`
    ...
    console.log('4. Проверяем UNIQUE ограничение на diagnostic_steps (problem_id, step_number)...');
    ...
    if (stepConstraintExists.rows.length === 0) {
      ...
      console.log('   ℹ️  UNIQUE constraint на diagnostic_steps уже существует');
    }
    ...
    console.error('❌ Ошибка при добавлении UNIQUE ограничений:', error.message);
    
    // Если ошибка связана с дубликатами, выводим подробную информацию
    if (error.code === '23505') {
      console.error('💡 Найдены дублирующиеся записи. Необходимо очистить данные перед применением ограничений.');
...
export const down = async () => {
  ...
  try {
    ...
    await query('DROP INDEX CONCURRENTLY IF EXISTS idx_tv_interfaces_name_unique_active;');
    await query('ALTER TABLE diagnostic_steps DROP CONSTRAINT IF EXISTS diagnostic_steps_problem_id_step_number_key;');
    await query('DROP INDEX CONCURRENTLY IF EXISTS idx_remotes_name_device_unique_active;');
    await query('DROP INDEX CONCURRENTLY IF EXISTS idx_problems_title_device_unique_active;');
    await query('DROP INDEX CONCURRENTLY IF EXISTS idx_devices_name_unique_active;');
