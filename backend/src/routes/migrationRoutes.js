...
router.post('/execute/:filename', async (req, res) => {
    ...
    try {
        ...
        console.error(`❌ API: Error executing migration ${filename}:`, error.message);
        res.status(500).json({
            success: false,
...
router.get('/check-structure', async (req, res) => {
    try {
        ...
        // Check each table
        const tables = ['devices', 'remotes', 'tv_interfaces', 'users', 'problems', 'diagnostic_steps', 'tv_interface_marks', 'remote_marks'];
