...
async function checkDatabaseStructure() {
    ...
    try {
        ...
        // Check TV interfaces
        const tvInterfaces = await query("SELECT COUNT(*) as count FROM tv_interfaces");
        ...
        // Check markings
        const tvMarks = await query("SELECT COUNT(*) as count FROM tv_interface_marks");
        console.log(`🎯 TV Interface Marks: ${tvMarks.rows[0].count} records`);
        
        const remoteMarks = await query("SELECT COUNT(*) as count FROM remote_marks");
